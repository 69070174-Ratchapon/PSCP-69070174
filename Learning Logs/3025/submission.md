...
```
รับ input มา 2 อย่างคือ month , day ตามลำดับ ต้องoutputเป็นฤดู4ฤดู ฤดูใบไม้ผลิ (spring) ฤดูร้อน (summer) ฤดูใบไม้ร่วง (fall) หรือฤดูหนาว (winter) แต่ถ้าเดือนนั้นหาร3ลงตัวและเป็นวันที่21เป็นต้นไปจะเปลี่ยนฤดูเป็นฤดูถัดไปแล้ว
```
...
```
ใช้เงื่อนไขคือถ้าเดือนที่หาร3ลงตัว ก็ทำการเข้าเงื่อนไขต่อว่าถ้าวันของเดือนนั้นเป็นวันตั้งแต่วันที่21 ก็ทำการให้เปลี่ยนฤดูเป็นฤดูถัดไป จากwinter to spring, spring to summer, summer to fall, fall to winter แต่ถ้าไม่ใช่ก็จะเป็นฤดูเดิมโดยเดือน1,2,3(ไม่เกินวันที่20) เป็นwinter ,4,5,6(ไม่เกินวันที่20) เป็น spring,7,8,9(ไม่เกินวันที่20) เป็น summer ,10,11,12(ไม่เกินวันที่20) เป็น fall
```
...
```
ใช้เหมือนแผนแรก โดยการเข้าเงื่อนไขแรกคือ if not month % 3 else คือเดือนหาร3ไม่ลงตัว 
```
...
```
ทดสอบว่าถ้าเดือนหาร3ไม่ลงตัวถูกต้องมั้ย?
```
...
```
ทดสอบว่าถ้าเป็นวันที่21และเดือนที่3หารลงตัว
```
...
```
วันมากกว่าหรือเท่ากับวันที่21 แต่เดือนหาร3ไม่ลงตัว
```
