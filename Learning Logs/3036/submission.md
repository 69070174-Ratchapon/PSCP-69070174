...
```text
รับค่ามาแล้วก้ทำการคำนวนว่าจะทำการเคลื่อนไหวกี่ครั้ง จากจุด1ไปจุดที่รับค่ามา
```
...
```text
ดูแค่สามเหลี่ยนที่โจทย์ให้มาแต่มีเยอะกว่านี้เลยต้องเปลี่ยนวิธี
```
...
```text
สังเกตุว่าด้านขวาของสามเหลี่ยมจะเป็นn^2เช่น1,4,9,16,25,... ถ้าเป็น1 แสดงว่า 0แล้วก็ทำการใช้while เพื่อcheckว่าได้ผลลัพธ์ยัง โดยข้างในwhile โดยเริ่มจากเช็คว่าอยุ่ระหว่างชั้นนั้นหรือป่าวโดยการใช้pow(expo,2) >= x >= pow(expo,2) - (2 * (expo - 1)): โดยexpoเริ่มที่2 ซึ่งถ้ามันอยู่ชั้นที่2 มันจะอยู่ระหว่าง 4 กับ 2 แล้วก้ไปเช็คต่อว่า ตัวinputนั้นเป็นเลขคู่หรือเลขคี่ แล้วก็ทำการดูว่าตัวอินพุทเป็นเลขคู่หรือเลขคี่ โดยถ้าexpo = 2 จะอยู่จะหว่าง 2กับ4 แล้ว สังเกติว่าถ้าinput = 3(เป็นเลขคี่) ก้ทำการขยับ 1 ครั้ง แต่ถ้าเป็นเลขคู่ก็ขยับ2ครับ ต่อมามาดูที่expo = 3 ก้ทำการแทนเข้าไปใน pow(expo,2) >= x >= pow(expo,2) - (2 * (expo - 1)): ก็คือจะเป็นการเช็คว่าอยู่ระหว่าง5กับ9มั้ย พอได้แล้วก็ทำการเช็คว่าเป็นเลขคู่หรือคี่ โดยถ้าเป็นเลขคู่ขยับ3ครั้ง คี่ขยับ4 โดยสังเกตุว่าจะสลับกันระหว่างชั้น2กับ3 โดยชั้นแรกถ้าเป็นเลขคี่จะขยับเป็นเลขจำนวนคี่ครั้ง แต่พอชั้นที่2เป็นเลขคี่จะเป็นคู่ครั้ง เท่ากับว่าเราก็ต้องทำให้มันสลับกัน และโดยชั้นที่2กับ3ขยับห่างกันอย่างน้อย2ครั้ง ถ้าไม่เข้าเงื่อนไขก็เลยบวกresultด้วย2(ตอนแรกresult = 1)
...
```text
เช็คว่าถ้าเป็นจุดเริ่มต้นต้องเป็นกี่ครั้ง
```
...
```text
เป็นเลขคู่ ที่อยุ่ระหว่าง2กับ4
```
